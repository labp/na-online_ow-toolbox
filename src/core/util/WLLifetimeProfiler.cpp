...
std::string WLLifetimeProfiler::getName() const
{
    return CLASS;
}

size_t WLLifetimeProfiler::getCloneCounter() const
{
    return m_clones;
}

WLLifetimeProfiler::AgeT WLLifetimeProfiler::getAge() const
{
    return m_timeProfiler->getMilliseconds();
}

void WLLifetimeProfiler::pause()
{
    m_timeProfiler->stop();
}
