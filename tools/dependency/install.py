...
All installer must implement AInstaller.
...
class Installer(AInstaller):
    ...
    def install(self):
        if AInstaller.ask_for_execute("Install Qt5 Framework"):
            call("python install_qt5_static.py", shell=True)
            
        print
    
        if AInstaller.ask_for_execute("Install MNE-CPP"):
            call("python install_mne.py", shell=True)
    
        print
    
        if AInstaller.ask_for_execute("Install FielTrip Buffer"):
            call("python install_ft_buffer.py", shell=True)
        
        return True


if __name__ == "__main__":
    installer = Installer("/tmp")
